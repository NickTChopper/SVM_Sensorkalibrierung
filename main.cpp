...
int main(){
    ...
    for(unsigned int i = 0; i < filenames_wheels.size(); i++){
        ...
        if(!wheel_image.data){
            ...
            for (int i = 0; i < wheel_image.rows; i++) {
                for (int j = 0; j < wheel_image.cols; j++) {
                    training_mat.at<float>(file_number,ii++) = wheel_image.at<uchar>(i,j);
                }
            }
    ...
    for(unsigned int i = 0; i < filenames_non_wheels.size()-2; i++){
        ...
        if(!non_wheel_image.data){
            ...
            int ii = 0; // Current column in training_mat
            for (int i = 0; i < non_wheel_image.rows; i++) {
                for (int j = 0; j < non_wheel_image.cols; j++) {
                    training_mat.at<float>(file_number,ii++) = non_wheel_image.at<uchar>(i,j);
                }
            }
    ...
    Mat sample_mat(1, amount_of_pixels, CV_32FC1);
    ...
    for( int j = 0; j<filenames_test_data.size(); j++){
        ...
        int ii = 0; // Current column in training_mat
        for (int i = 0; i < sample_mat_file.rows; i++) {

            for (int j = 0; j < sample_mat_file.cols; j++){
                sample_mat.at<float>(0,ii++) = sample_mat_file.at<uchar>(i,j);
                }
